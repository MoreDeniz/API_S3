...
selectEl.addEventListener("change", function ({target}) {
    // В checkSelect лежит то что мы выбрали
    const category = target.value;
    // очищаем
    ulEl.innerHTML = productsData.filter(product => product.category === category)// Бежим по массиву
...
//   // отфильтруем массив
//   const html = productsData.filter(product => product.category === category)// Бежим по массиву
//   // возвращает массив строк
//   .map(product => `<li><p>${product.name}</p><p>${product.category}</p></li>`)
//   // соединим ""
//   .join("");

//   //передаём сюда нашу строку
//   ulEl.insertAdjacentHTML("beforebegin", html);
